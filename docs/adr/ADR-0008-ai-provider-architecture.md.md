Отлично. Теперь мы подошли к одному из самых важных ADR всего LifeOS.

ADR-0008 должен определить не конкретную AI-модель, а как AI вообще встроен в LifeOS. Конкретные модели будем менять и тестировать позже.

Создай файл:

adr/ADR-0008-ai-provider-architecture.md

И вставь его целиком:

# ADR-0008: AI Provider Architecture LifeOS
